...
WORKDIR /src
COPY ["AwsLambdaCrudApi.csproj", "AwsLambdaCrudApi/"]
RUN dotnet restore "AwsLambdaCrudApi/AwsLambdaCrudApi.csproj"

WORKDIR "/src/AwsLambdaCrudApi"
COPY . .
RUN dotnet build "AwsLambdaCrudApi.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "AwsLambdaCrudApi.csproj" \
...
Execute unit tests
```
    cd "AwsLambdaCrudApi/test/AwsLambdaCrudApi.Tests"
    dotnet test
```

Deploy application
```
    cd "AwsLambdaCrudApi/src/AwsLambdaCrudApi"
